...
export function ModelSelectorSheet({ visible, models, loading, error, modelStatus = null, modelStatusLoading = false, currentModel, onClose, onRetry, onSelect }: ModelSelectorSheetProps) {
  ...
  const renderModel = ({ item: entry }: { item: ModelCatalogEntry }) => {
    ...
    return <View style={[styles.modelRow, { backgroundColor: selected ? theme.colors.accentSoft : "transparent", borderColor: selected ? theme.colors.accentBorder : theme.colors.border }]}>
      <Pressable testID={`model-option-${entry.provider}-${entry.id}`} accessibilityRole="button" accessibilityLabel={t("model.use", { name: modelDisplayName(entry) })} accessibilityState={{ selected }} onPress={() => selectEntry(entry, levels.length > 1 ? selectedLevel : undefined)} style={({ pressed }) => ({ flexDirection: "row", alignItems: "flex-start", gap: 10, opacity: pressed ? 0.72 : 1 })}>
      ...
      {levels.length > 1 ? <View style={{ marginTop: 8, paddingLeft: 46 }}>
        <Pressable accessibilityRole="button" accessibilityLabel={t("model.thinkingA11y", { level: formatThinkingLevel(selectedLevel) })} accessibilityState={{ expanded: thinkingOpen }} onPress={() => setThinkingOpenFor(thinkingOpen ? null : key)} style={({ pressed }) => [styles.thinkingTrigger, { backgroundColor: pressed ? theme.colors.surfacePressed : theme.colors.background, borderColor: thinkingOpen ? theme.colors.accentBorder : theme.colors.border }]}>
          <AppIcon name="brain" size={14} color={theme.colors.accent} /><Text style={[typography.micro, { color: theme.colors.textSecondary, flex: 1 }]}>{t("model.thinking", { level: formatThinkingLevel(selectedLevel) })}</Text><AppIcon name="chevron-down" size={13} color={theme.colors.textMuted} style={{ transform: [{ rotate: thinkingOpen ? "180deg" : "0deg" }] }} />
        </Pressable>
        {thinkingOpen ? <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 6, paddingTop: 7, paddingBottom: 2 }}>{levels.map((level) => { const active = level === selectedLevel; return <Pressable key={level} accessibilityRole="button" accessibilityLabel={t("model.useThinking", { level: formatThinkingLevel(level) })} accessibilityState={{ selected: active }} onPress={() => selectEntry(entry, level)} style={({ pressed }) => ({ minHeight: 32, paddingHorizontal: 10, borderRadius: 8, justifyContent: "center", borderWidth: 1, borderColor: active ? theme.colors.accentBorder : theme.colors.border, backgroundColor: active ? theme.colors.accentSoft : pressed ? theme.colors.surfacePressed : theme.colors.surface })}><Text style={[typography.micro, { color: active ? theme.colors.accent : theme.colors.textMuted }]}>{formatThinkingLevelShort(level)}</Text></Pressable>; })}</ScrollView> : null}
      </View> : null}
    </View>;
  };

  return <AdaptiveSheet visible={visible} title={t("model.title")} subtitle={t("model.subtitle")} onClose={onClose} scrollable={false} contentStyle={{ flex: 1, minHeight: 0 }} fullHeight testID="chat-model-selector-sheet">
    <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
      <View style={{ flex: 1 }}><SearchField value={query} onChangeText={setQuery} placeholder={t("model.search")} /></View>
      {modelStatusLoading ? <ActivityIndicator size="small" color={theme.colors.accent} /> : null}
    </View>
    <Pressable testID="model-option-automatic" accessibilityRole="button" accessibilityLabel={t("model.automaticA11y")} accessibilityState={{ selected: currentModel === null }} onPress={() => { onSelect(null); setThinkingOpenFor(null); }} style={({ pressed }) => [styles.automaticRow, { backgroundColor: pressed ? theme.colors.surfacePressed : currentModel === null ? theme.colors.accentSoft : theme.colors.surfaceRaised, borderColor: currentModel === null ? theme.colors.accentBorder : theme.colors.border }]}>
      ...
      <View style={styles.modelText}><Text style={[typography.bodyMedium, { color: theme.colors.text }]}>{t("model.automatic")}</Text><Text numberOfLines={1} style={[typography.caption, { color: theme.colors.textMuted, marginTop: 2 }]}>{t("model.automaticDetail")}</Text></View>
