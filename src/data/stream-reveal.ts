...
/**
 * Presentation-only pacing for one streaming text item. Cohub's snapshots stay authoritative: the
 * controller never invents text, it only decides when a prefix becomes visible to the renderer.
 * Appends reveal grapheme by grapheme; a rewrite or a new item resets without replaying.
 *
 * The renderer is the native Markdown view, so this no longer re-parses anything itself — commits
 * are throttled because every commit re-renders the trailing block natively. The interval widens as
 * the active block grows, and while input is active the reveal rate tracks the arrival rate with a
 * small time-based buffer, so the display never trails a fast stream by more than that buffer.
 */
export class StreamRevealController {
  ...
  setTarget(source: string) {
    ...
    if (!this.initialized) {
      ...
      // First content is authoritative; there is no prior text to reveal from.
      this.flush(source);
      return;
    }
    ...
    // An append can extend the trailing grapheme (combining mark, ZWJ), so re-segment from that
    // unit instead of trusting the previous boundaries.
    const tail = this.units.at(-1);
    const tailStart = tail?.offset ?? 0;
    if (tail) this.units.length -= 1;
    segment(source.slice(tailStart)).forEach((unit) => {
      this.units.push({ offset: tailStart + unit.offset, length: unit.length });
    });
    // The last old unit may now be extended, so step back onto it instead of treating the appended
    // graphemes as already displayed.
  ...
  /** Show the full target immediately (completion, rewrite, teardown). */
  flush(source = this.source) {
    this.clearTimer();
    this.initialized = true;
    this.source = source;
    // Skip segmentation: a future append re-segments from the tail anyway, and non-streaming mounts
    // must not pay O(source) per history row.
    this.units = [];
    this.nextUnit = 0;
