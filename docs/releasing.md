...
1. `CI` validates every PR and push to `main`, and exports Android and iOS JavaScript bundles in parallel with the quality checks.
...
- The publishing credential is stored in this repository's `OTA_API_KEY` Actions secret. It is not an app environment variable and must never use an `EXPO_PUBLIC_*` name. Keep the Yaota publisher pinned to a reviewed commit.
