...
About > Application and the in-app update banner scan Yaota (`https://mobile.talesofai.com/api/apks`) for the newest signed APK for the device, so they only prompt when a new native package is required; JS-only releases stay silent and arrive through OTA. Native Tag Release still attaches the four ABI APKs to the GitHub Release for archival download, then uploads the same files to Yaota. The app selects the device ABI, downloads the APK from Yaota into its private cache, verifies the published size and SHA-256 digest, and grants Android's package installer temporary access through a `content://` URI. The primary update action does not open a browser. Missing or malformed digests block installation.
