...
- Global Chats poll at 15 seconds while active, 30 seconds when visible and idle, and 60 seconds offscreen. Source-filtered and Space/panel Chat lists poll at 15 seconds. Head reconciliation retains loaded older pages and tail cursors, and the Chat lists enable LegendList data-change anchoring.
...
- Queued and stop-requested turns now count as active in the existing Running presentation. The recency filter itself is unchanged.
...
- Automatic status reads do not activate empty-list loading placeholders. File refresh failures retain existing entries and show an error. Account-generation checks and scheduler disposal reject obsolete work.
