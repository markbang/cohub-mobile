...
function ChatContent({ sessionId, initialTurnSequence, initialTurnId }: { sessionId: string; initialTurnSequence: number | null; initialTurnId: string | null }) {
  ...
  // Stable identity keeps memoized MessageBubble rows out of the streaming re-render path.
  const handleMessageLongPress = useCallback((text: string, origin: { x: number; y: number }) => {
    setMessageAction({ text, ...origin });
  }, []);
