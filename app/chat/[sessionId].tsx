...
function ChatContent({ sessionId, initialTurnSequence, initialTurnId }: { sessionId: string; initialTurnSequence: number | null; initialTurnId: string | null }) {
  ...
  const [sendTransition, setSendTransition] = useState<{ text: string; sourceX: number; sourceY: number; targetX: number; targetY: number; targetHeight: number } | null>(null);
  ...
  const [transitionMessageState, setTransitionMessage] = useState<MessageRecord | null>(null);
  ...
  const transitionMessage = sendTransition ? timeline.find((item) => item.meta?.optimistic === true && sendTransition.text === messageText(item)) ?? transitionMessageState : null;
  ...
  const submit = async () => {
    ...
    composerRef.current?.measureInWindow((composerX, composerY, composerWidth, composerHeight) => {
      listContainerRef.current?.measureInWindow((listX, listY) => setSendTransition({ text: transitionText, sourceX: composerX - listX + Math.max(16, composerWidth - 60), sourceY: composerY - listY + Math.min(composerHeight, 58) / 2 - 5, targetX: 0, targetY: 0, targetHeight: 0 }));
    });
    setInput("");
    setAttachments([]);
    try {
  ...
  return <Screen keyboard>
    ...
    <SpacePanels key={spaceId || sessionId} spaceId={spaceId} spaceName={spaceName} sessions={spaceSessions} client={client} activePanel={activePanel} onActivePanelChange={setActivePanel} onOpenSession={(nextSessionId, target) => router.push({ pathname: "/chat/[sessionId]", params: { sessionId: nextSessionId, ...(target?.turn != null ? { turn: String(target.turn) } : {}), ...(target?.turnId ? { turnId: target.turnId } : {}) } })} onNewChat={() => { if (spaceId) router.push({ pathname: "/chat/[sessionId]", params: { sessionId: "new", spaceId } }); }} onOpenFile={(path) => { if (spaceId) router.push({ pathname: "/space/[spaceId]/file", params: { spaceId, path } }); }} onOpenFilesPage={() => { if (spaceId) router.push({ pathname: "/space/[spaceId]/files", params: { spaceId } }); }}>
      <View style={{ flex: 1, minHeight: 0 }}>
        ...
        {sendTransition && transitionMessage && sendTransition.targetHeight > 0 ? <SendTransitionOverlay transition={sendTransition} message={transitionMessage} spaceId={spaceId} availableWidth={listWidth || undefined} onCopy={handleCopyMessage} /> : null}
...
function SendTransitionOverlay({ transition, message, spaceId, availableWidth, onCopy }: { transition: { text: string; sourceX: number; sourceY: number; targetX: number; targetY: number; targetHeight: number }; message: MessageRecord; spaceId?: string; availableWidth?: number; onCopy: (text: string) => void }) {
  ...
  const style = useAnimatedStyle(() => ({
    ...
    transform: [
      { translateX: (transition.targetX - transition.sourceX) * travel.value },
      { translateY: (transition.targetY - transition.sourceY) * travel.value },
      { scale: interpolate(pop.value, [0, 0.55, 1], [0.94, 1.045, 1]) },
    ],
  }));
  return <Reanimated.View pointerEvents="none" style={[{ position: "absolute", left: transition.sourceX, top: transition.sourceY - 22, zIndex: 20, elevation: 6 }, style]}><MessageBubble message={message} local onCopy={onCopy} spaceId={spaceId} availableWidth={availableWidth} floating /></Reanimated.View>;
}
