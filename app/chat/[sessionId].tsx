...
function ChatContent({ sessionId, initialTurnSequence, initialTurnId }: { sessionId: string; initialTurnSequence: number | null; initialTurnId: string | null }) {
  ...
  const transitionMessage = sendTransition ? timeline.find((item) => item.role === "user" && sendTransition.text === messageText(item) && item.createdAt >= sendTransition.startedAt) ?? transitionMessageState : null;
  ...
  const submit = async () => {
    ...
    composerRef.current?.measureInWindow((composerX, composerY, composerWidth, composerHeight) => {
      listContainerRef.current?.measureInWindow((listX, listY) => setSendTransition({ text: transitionText, startedAt: transitionStartedAt, sourceX: composerX - listX + Math.max(16, composerWidth - 60), sourceY: composerY - listY + Math.min(composerHeight, 58) / 2 - 5, targetX: 0, targetY: 0, targetHeight: 0 }));
    });
    setInput("");
    setAttachments([]);
    try {
