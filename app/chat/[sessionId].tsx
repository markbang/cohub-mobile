...
import { ActivityIndicator, Alert, FlatList, Modal, Pressable, Share, Text, TextInput, View, useWindowDimensions, type ViewToken } from "react-native";
...
function ChatContent({ sessionId, initialTurnSequence, initialTurnId }: { sessionId: string; initialTurnSequence: number | null; initialTurnId: string | null }) {
  ...
  const showMessageActions = useCallback((message: MessageRecord) => {
    const text = messageText(message);
    const actions = [
      { text: t("chat.copy"), onPress: () => handleCopyMessage(text) },
      ...(typeof message.meta?.turnId === "string" && isTerminalTurnStatus(view.turns.find((turn) => turn.id === message.meta?.turnId)?.status)
        ? [{ text: t("chat.fork"), onPress: () => void forkMessage(message) }]
        : []),
      { text: t("common.cancel"), style: "cancel" as const },
    ];
    Alert.alert(t("chat.more"), undefined, actions);
  }, [forkMessage, handleCopyMessage, t, view.turns]);
