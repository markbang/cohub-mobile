...
function SendBubbleMotion({ transition, transitionKey, message, spaceId, onCopy, bubbleRef, onBubbleLayout }: { transition: { sourceX: number; sourceY: number; targetX: number; targetY: number; targetHeight: number }; transitionKey: string; message: MessageRecord; spaceId?: string; onCopy: (text: string) => void; bubbleRef: React.RefObject<View | null>; onBubbleLayout?: () => void }) {
  const pop = useSharedValue(0);
  const travel = useSharedValue(0);
  const animationStarted = useRef(false);
  useEffect(() => {
    recordDebugEvent("chat.send_transition.mounted", { transitionKey });
    return () => recordDebugEvent("chat.send_transition.unmounted", { transitionKey });
  }, [transitionKey]);
  useEffect(() => {
  ...
  }, [pop, travel, transition.sourceX, transition.sourceY, transition.targetHeight, transition.targetX, transition.targetY, transitionKey]);
