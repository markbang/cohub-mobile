...
import { ActivityIndicator, BackHandler, FlatList, Modal, Pressable, Share, Text, TextInput, View, useWindowDimensions, type ViewToken } from "react-native";
...
function ChatContent({ sessionId, initialTurnSequence, initialTurnId }: { sessionId: string; initialTurnSequence: number | null; initialTurnId: string | null }) {
  ...
  const requestTextSelection = useCallback((messageId: string) => {
  ...
  return <Screen keyboard>
    <SpacePanels key={spaceId || sessionId} spaceId={spaceId} spaceName={spaceName} sessions={spaceSessions} client={client} activePanel={activePanel} swipeEnabled={selectingMessageId === null} onActivePanelChange={setActivePanel} onOpenSession={(nextSessionId, target) => router.push({ pathname: "/chat/[sessionId]", params: { sessionId: nextSessionId, ...(target?.turn != null ? { turn: String(target.turn) } : {}), ...(target?.turnId ? { turnId: target.turnId } : {}) } })} onNewChat={() => { if (spaceId) router.push({ pathname: "/chat/[sessionId]", params: { sessionId: "new", spaceId } }); }} onOpenFile={(path) => { if (spaceId) router.push({ pathname: "/space/[spaceId]/file", params: { spaceId, path } }); }} onOpenFilesPage={() => { if (spaceId) router.push({ pathname: "/space/[spaceId]/files", params: { spaceId } }); }}>
      <View style={{ flex: 1 }}>
        <DetailTopBar title={session ? displaySessionTitle(session) : "Chat"} subtitle={selectingMessageId ? "Long-press the message text to select" : spaceName} onBack={() => (selectingMessageId ? setSelectingMessageId(null) : router.back())} actions={selectingMessageId ? <Pressable accessibilityRole="button" accessibilityLabel="Done selecting text" onPress={() => setSelectingMessageId(null)} style={({ pressed }) => ({ minHeight: 38, paddingHorizontal: 14, borderRadius: 999, alignItems: "center", justifyContent: "center", backgroundColor: pressed ? theme.colors.surfacePressed : theme.colors.accentSoft })}><Text style={[typography.bodyMedium, { color: theme.colors.accent }]}>Done</Text></Pressable> : <><IconButton name="list-tree" label="Open conversation turns" size={38} onPress={() => setTurnNavigatorOpen(true)} disabled={view.turnIndex.length === 0 && view.loading} /><IconButton name="more" label="More actions" size={38} onPress={() => setMoreOpen(true)} /></>} />
