...
export default function NewChatScreen() {
  ...
  return <Screen>
    <TopBar title="New Chat" left={<Pressable accessibilityRole="button" accessibilityLabel="Close" onPress={() => router.back()} hitSlop={8} style={{ width: 40, height: 40, alignItems: "center", justifyContent: "center" }}><AppIcon name="close" size={22} color={theme.colors.textSecondary} /></Pressable>} />
    <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={{ padding: 16, paddingBottom: 30 }}>
      ...
      <View style={{ marginTop: 16, borderWidth: 1, borderColor: theme.colors.border, borderRadius: 14, overflow: "hidden", backgroundColor: theme.colors.surface }}>
        {state.spaces.map((space) => { const selected = selectedSpaceId === space.id; return <Pressable key={space.id} onPress={() => setSelectedSpaceId(space.id)} style={({ pressed }) => ({ flexDirection: "row", alignItems: "center", gap: 11, minHeight: 66, paddingHorizontal: 12, backgroundColor: selected ? theme.colors.accentSoft : pressed ? theme.colors.surfacePressed : "transparent", borderBottomWidth: 1, borderBottomColor: theme.colors.border })}><Avatar name={displaySpaceName(space)} uri={space.publicProfile?.avatarUrl} size={38} /><View style={{ flex: 1, minWidth: 0 }}><Text numberOfLines={1} style={[typography.bodyMedium, { color: theme.colors.text }]}>{displaySpaceName(space)}</Text><Text numberOfLines={1} style={[typography.caption, { color: theme.colors.textMuted, marginTop: 2 }]}>{space.description || "Ready for work"}</Text></View>{selected ? <AppIcon name="checkmark-circle" size={21} color={theme.colors.accent} /> : <View style={{ width: 21, height: 21, borderRadius: 11, borderWidth: 1, borderColor: theme.colors.borderStrong }} />}</Pressable>; })}
      ...
      {selectedSpace ? <View style={{ flexDirection: "row", alignItems: "center", gap: 8, marginTop: 13 }}><AppIcon name="information-circle-outline" size={15} color={theme.colors.textMuted} /><Text style={[typography.caption, { color: theme.colors.textMuted, flex: 1 }]}>This Chat will run inside {displaySpaceName(selectedSpace)}.</Text></View> : null}
      {error ? <Text style={[typography.caption, { color: theme.colors.danger, marginTop: 14 }]}>{error}</Text> : null}
      <PrimaryButton label="Start Chat" icon="arrow-forward" loading={creating} disabled={!selectedSpaceId} onPress={() => void start()} style={{ marginTop: 26 }} />
    </ScrollView>
  </Screen>;
}
