...
Sign in against the dev deployment from the login screen: the environment picker switches between prod and dev, and the choice is remembered on the device. `EXPO_PUBLIC_COHUB_ENV=dev` only changes the build-time default. When a build pins endpoints through `EXPO_PUBLIC_*` overrides, those take precedence over the selected environment.
